# source-extension-parity Specification

## Purpose
定义 Mihon 的源浏览、扩展生命周期与登录错误语义如何由固定原版证据约束并被当前 Android 与 Desktop consumer 共同消费，同时将 Desktop APK→JAR、文件事务和浏览器挑战等平台差异限制在有真实扩展证据的 adapter 内。
## Requirements
