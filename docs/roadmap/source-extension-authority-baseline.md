...
| --- | --- | --- | --- | --- | --- |
| 28 源列表 | `BrowseTab` → `AndroidSourceManager` 响应扩展变化，`GetEnabledSources` / `SourcesScreenModel` 投影 last-used→pinned→language | 无；两端分别消费同一 fixed-main fixture，不冒充 shared output | `AndroidSourceManager` → `GetEnabledSources` → `SourcesScreenModel` | `DesktopSourceManager` → `DesktopSourceListProjector` / `BrowseSourceListScreen` | 当前 Android consumer 与 Desktop consumer 均保持 reactive membership；Desktop 宽屏与 Voyager 仅为 platform adapter。状态为 `WIRED`，尚无最新跨平台运行时证据。 |
| 29 单源浏览 | `BrowseSourceScreenModel` → `SourcePagingSource` → `NetworkToLocalManga` / `SManga.toDomainManga()`，并在非 incognito 时写 last-used | `SourceMangaSearchService`、`SourceQueryState`、common `NetworkToLocalManga` / `SManga` | `BrowseSourceScreenModel` → Android `SourcePagingSource` | `SourceBrowseScreen` / `DesktopSourceQueryCoordinators` → `SaveSourceMangaForDetails` | canonical 持久化、数据库观察、空页错误和 incognito 边界已接入；键鼠布局是 platform adapter。状态为 `WIRED`。 |
