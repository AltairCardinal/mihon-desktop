...
**Platform boundary:** shared+desktop
**Estimated scope:** 8 files, 3800 lines
...
**Scope update:** 最终累计 8 个 shared+desktop 文件，+3657/-88（3745 changed lines）；本数值取代上方 waiver 的历史 7 文件/2534 行计数。新增范围仅为 `DesktopNetworkHelper` 的 production network-interceptor wiring，以及同一 policy 测试中的 final outbound Cookie-header/UA 配对交错矩阵；原 waiver 关于单一 challenge-recovery-policy 风险轴、不可拆分 production 链和 Task 5C 边界的理由保持不变。
