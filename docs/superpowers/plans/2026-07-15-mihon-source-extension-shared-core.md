...
**Platform boundary:** shared+desktop
**Estimated scope:** 6 files, 1095 lines
...
**Split waiver:** 实际 1095 changed lines（+1060/-35）中，805 行是新 shared session、initiator-bound Desktop completion ticket 及其状态/并发行为矩阵，290 行是既有 `DesktopCookieJar` 的 Cookie.matches/hostOnly/事务持久化与 barrier 回归。共享 success/cancel/timeout/commit 边界只有穿过真实 committer 与 jar 的整组验证、内存替换、临时文件原子落盘、失败恢复和锁可见性后才构成可交付能力；若拆开，shared/adapter 子任务可独立全绿但仍无法证明取消、持久化失败或并发读取不会半写/暴露凭据。超额来自同一 `login-session-atomicity` 风险轴及审查要求关闭的 5 个 Important，而非 UI、challenge policy 或无关重构。
