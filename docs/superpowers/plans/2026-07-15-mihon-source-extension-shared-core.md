...
- Repair verification: BUILD39 stopped in the fresh Windows suite after 1814 tests with one failure and two skips. `DesktopChallengeLoginWiringTest` received `UncaughtExceptionsBeforeTest`; its suppressed root cause was `IllegalStateException: Node has been removed` from a `DesktopPreference.changes()` Compose collector left by the preceding `SourceLastUsedWiringTest`. Closing the scene and cancel/joining its child Job does not prove the old composition applied disposal before the test removes its Preferences node, matching the lifecycle boundary already proven in Task 7D37. Use this Task's single repair round: while the node is still valid, replace the mounted scene content with an empty composition and render that disposal, then close and cancel/join before callers remove the node. Preserve every convergence and behavior assertion. Run SourceLastUsed followed by ChallengeLogin on Windows and macOS, Spotless and the guard, then repair re-review. Do not catch the Preferences error, retain the node, sleep, serialize tests or change production.
- Repair evidence: commit `e490d1fcd`. `MountedBrowseScene.cleanup()` now installs and renders empty content while Preferences are valid, then closes the scene and cancel/joins its child Job; both mounted callers await cleanup before removing their node. The Windows strict SourceLastUsed 3/3 then ChallengeLogin 14/14 sequence passed, and the exact committed snapshot plus SHA-identical macOS overlay passed the same 17/17 sequence. Spotless, diff check and Comet guard passed. Repair re-review APPROVED with Critical/Important/Minor `0/0/0`, confirming the change applies old-composition disposal without altering D42 convergence, diagnostics or behavior assertions. BUILD39 remains rejected; a fresh full verify is still required.

##### Task 7D43: Extension presentation model-to-Compose convergence

- Risk axis: `extension-presentation-compose-convergence`
- Platform boundary: `verification`
- Estimated scope: `1 file, 100 lines`
- Verification: BUILD40 stopped in the fresh Windows suite after 1814 tests with one failure and two skips. `ExtensionPresentationUiTest.production content renders local empty data with failure and retries` first observed the real ScreenModel install step reach `Downloading`, rendered one frame, then failed the exact downloading-semantics assertion at line 398. The same long production-content test has further state-to-Compose boundaries for errors, retry/cancel, trust dialogs, update-all feedback and language filters that also wait for model state but assume one frame is enough. Treat the full-suite failure as RED and keep production unchanged. Replace every such asynchronous single-frame assumption in this test with the existing bounded five-second render/yield convergence on the exact text/action semantics before asserting or clicking; retain synchronous one-frame interactions where no asynchronous state boundary exists. Preserve the real `ExtensionListScreen`, `ExtensionsScreenModel`, production presentation port, installed/catalog flows, exact projection identity/deduplication, begin-install/confirm/discard call counts, update-all cancellation semantics, error copy, trust dialogs, filter options and refresh retry. Do not sleep, enlarge the existing five-second budget, bypass/mutate ScreenModel state, remove assertions, weaken copy/action identity, serialize tests or change production. Run the focused presentation class and adjacent extension details/copy/preferences matrix on Windows and the exact-patch macOS snapshot, Spotless, Comet guard and independent review. BUILD40 remains rejected; after closure allocate the next BUILD for Step 6.
- Evidence: commit `482e1ee86`. The long production-content fixture now uses a local five-second render/yield `awaitAction` whose Text and ContentDescription matching is identical to the real `click` helper, and the existing `awaitText` for feedback. Initial empty/catalog projection, both error/retry and downloading/cancel paths, trust confirm/discard, update-all cancellation/completion and language-filter dialog boundaries converge before their unchanged clicks/assertions; synchronous tab/toggle/apply interactions remain single-frame. Projection identity/deduplication, exact install/trust call counts, error copy, update-all semantics and filter/refresh assertions are unchanged. Windows XML passed `ExtensionPresentationUiTest` 7/7, `DesktopExtensionDetailsActionCopyTest` 1/1 and `ExtensionDetailsPreferencesWiringTest` 4/4; the exact committed snapshot plus SHA-identical macOS overlay passed the same 12/12 matrix. Spotless, diff check and Comet guard passed. Independent review APPROVED with Critical/Important/Minor `0/0/0`; an initial matrix-count question was resolved against the explicit 7D38 definition of the same details/action-copy/preferences 7+1+4 boundary, without expanding to the unrelated list-copy projection contract. BUILD40 remains rejected; the next fresh full verify is required.

  Final Step 6 evidence: exact code baseline `ae3a2346f`. On Windows, the repository build script allocated and validated `Mihon Desktop 0.11.14.41.ae3a234`; the canonical unpackaged EXE at `D:\Shell\Github\mihon\app-desktop\tmp\mihon-dist\main\app\Mihon Desktop\Mihon Desktop.exe` had the current build mtime and exact window title. The official full-test run passed; the final repaired extension presentation/details/preferences matrix passed 7/7 + 1/1 + 4/4, challenge handling passed 14/14, and the integration-enabled platform credential matrix exercised real Windows DPAPI for 9/9. Desktop smoke passed 88/88. The packaged EXE Test Mode returned healthy and completed the production Browse tab -> Extension list -> `extension_search` (`wired-query`) path with the final screen, query and snapshot reported by `/test/state`.

  macOS used a temporary clone of the same `ae3a2346f` snapshot and the matching pre-build version overlay. Official `full-tests` passed: extension transaction 42/42, global-search navigation 3/3, platform credentials 8/8 plus one narrowly expected headless Keychain skip, source-last-used 3/3, extension presentation/details/preferences 12/12, and challenge handling 14/14. The repository build script then allocated `Mihon Desktop 0.11.14.42.ae3a234`, built and deployed `/Applications/Mihon Desktop.app`; its default JVM smoke passed 88/88 and the deployed Test Mode completed the same Browse -> Extension -> search production path. The one Keychain skip is limited to SSH headless execution after the real `security` probe returned exit 36 (`User interaction is not allowed`); it does not claim GUI Keychain acceptance and remains a later platform-integration runtime boundary. Remote clone, bundle, logs and Test Mode processes were cleaned up. BUILD36 through BUILD40 remain rejected historical attempts and are not acceptance evidence.

##### Task 7D44: JVM transaction parent completion lifecycle

- Risk axis: `jvm-transaction-parent-lifecycle`
- Platform boundary: `desktop`
- Estimated scope: `2 files, 100 lines`
- Verification: final thorough review of `852221f42..f436c48de` found that each outer JVM transaction registers `parentJob.invokeOnCompletion { controlJob.cancel() }` without disposing the returned handle. A long-lived application or ScreenModel coroutine that repeatedly calls `DatabaseHandler.await(inTransaction = true)` therefore retains one cancelled control Job and closure per completed transaction until the parent itself ends. First add a production-path RED using one deliberately long-lived parent coroutine across multiple real transactions: while each transaction is active its control lifecycle must be structurally owned by that parent, and after each completion no transaction control child/registration may remain before the next iteration. GREEN must use structured Job ownership or explicitly dispose the exact completion handle on the final state release, preserve caller cancellation, same-handler nesting, cross-handler nesting, JDBC thread confinement and outer serialization, and cover acquisition failure cleanup. Do not use reflection over coroutine internals, sleep, a test-only production hook or a global registry. Run the complete `JvmDatabaseHandlerTest`, the affected source/global-search transaction regressions, root Spotless, diff check and Comet guard, then one independent repair re-review. Because this changes the code baseline after BUILD41/42, reopen Step 6 and repeat exact Windows/macOS verify only after this repair is approved.

  Initial review evidence: final review reported Critical/Important/Minor `0/1/0`. The Important traces from repeated outer `JvmDatabaseHandler.withTransaction` calls to `createTransactionState`, where the parent completion node is retained although `JvmTransactionState.release()` only cancels the detached control Job. No other Critical/Important/Minor finding was reported; fixed-main authority, shared consumer wiring, Desktop adapters and compat evidence were accepted. Task 7 and Step 7 remain open until this lifecycle defect is repaired, re-reviewed and the new exact code baseline is verified.

  Repair evidence: implementation `d9eb4b2dd` replaced the detached `Job()` plus undisposed parent completion callback with `Job(coroutineContext[Job])`. A real JDBC test using one long-lived parent across three transactions first failed on the old implementation (`expected 2 / actual 1` active children), then proved every structured control Job is present only during its transaction and detached before the next round. The implementation passed `JvmDatabaseHandlerTest` 8/8, data JVM 21/21, GlobalSearch 2/2, SourceBrowse 3/3, Spotless, diff check and Comet guard. Initial repair review accepted the lifecycle fix but found the planned rejected-dispatch cleanup path untested. The sole review repair `39248dd2a` added a reject-once dispatcher test: the production wrapper/cause are exact, the failed acquisition leaves no parent child, and the same handler/mutex then commits a real JDBC write. Removing the continuation cancellation callback produced the expected mutation RED with a retained control child; after restoration, handler 9/9 and data JVM 22/22 passed with Spotless, diff check and guard. Repair re-review APPROVED with Critical/Important/Minor `0/0/0`; production contains no mutation or further change.

##### Task 7D45: Download DI lifecycle fixture transport isolation

- Risk axis: `desktop-download-lifecycle-fixture-transport`
- Platform boundary: `verification`
- Estimated scope: `1 file, 80 lines`
- Verification: the post-7D44 Windows BUILD42 exact baseline passed full tests, fixed EXE, smoke and Test Mode, but the exact macOS `full-tests` run finished 1817 tests with one expected Keychain skip and one failure. `DesktopDiWiringTest.reinitializing stops active download worker before installing fresh context` timed out at line 493 while waiting two seconds for its auxiliary `ServerSocket` thread to receive a localhost request; the intended cancellation/finally/DI assertions had not begun, the other eight DI tests passed, and cleanup then logged the fixture socket closing. This test verifies that production reinitialization waits for the real download worker's `NonCancellable` cleanup before replacing manager/database wiring; it does not verify HTTP transport. Remove the OS socket/thread dependency by making the already injected `DownloadFileOperations.execute` return a valid in-memory successful OkHttp `Response`, signal exact execute/read phases, and retain the real manager worker, persistent queue, DOWNLOADING transition, cancellation, blocked finally, `stopAndJoin`, old database closure, fresh DI identities and unchanged old queue. Do not change production, increase timeouts, sleep, mock the manager/context or weaken any lifecycle assertion. Run the focused test repeatedly on Windows and once on an exact-patch macOS clone, the complete DI class, Spotless, diff check and Comet guard, then independent review. A fresh exact Windows/macOS Step 6 is still required after the reviewed test commit because the packaged version hash must match final HEAD.

  Initial failure evidence: Windows `Mihon Desktop 0.11.14.42.527e07d` passed the official full-tests, new transaction 9/9, smoke 88/88, exact title/mtime and Browse -> Extension -> search Test Mode path. The macOS clone at exact `527e07d99` plus BUILD42 overlay reported 1817 tests / 1 skip / 1 failure / 0 errors; the sole failure was the DI class at 34.836 seconds with `TimeoutCancellationException` from its two-second `requestStarted.await()`, while the adjacent eight tests completed in 0.201–0.474 seconds. BUILD43, deployment, smoke and Test Mode were correctly not run. This failed verify remains diagnostic evidence only.
...
- [x] **Step 7: 独立批次与最终审查**
  ...
  Evidence: the final review covered fixed-main authority, shared source/extension state and both production consumers, Desktop install/login/challenge adapters, compat inventory/evidence, high-risk Android transaction paths and change/checkoff consistency. It initially returned Critical/Important/Minor `0/1/0` for Task 7D44's parent Job retention. After implementation and its single test-coverage repair, the same independent reviewer APPROVED with `0/0/0`; no other finding remains. Step 6 alone remains open because the reviewed code baseline postdates BUILD41/42 and therefore requires a fresh exact Windows/macOS verify.

  Post-review verification exposed only Task 7D45's unrelated socket fixture dependency before macOS reached the intended lifecycle assertions. Its test-only repair was independently reviewed and APPROVED with `0/0/0`; it does not alter the already approved production diff. Step 7 is therefore closed again, while Step 6 remains open until the new exact HEAD passes both platforms.
