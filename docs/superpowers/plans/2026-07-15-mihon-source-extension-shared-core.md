...
##### Task 7D30: Fixed-main transaction serialization for source materialization

- Risk axis: `global-search-persistence-serialization`
- Platform boundary: `desktop`
- Estimated scope: `3 files, 100 lines`
- Verification: fixed-main Android Mihon serializes outer SQLDelight transactions with `transactionMutex`, while Desktop currently lets multiple source results call the same JDBC persistence path concurrently and converts one transaction failure into a terminal `CanonicalSearchResult.Failure`. First add a deterministic materializer RED: block source A inside persist, start source B in the same sync, and fail if B enters before A releases; old code must publish a failure/missing content. GREEN adds one persistence mutex around only the materializer's `persist` call, preserving parallel source HTTP requests, generation/attempt CAS, cancellation, close and typed publication. Add the terminal result/cause to the existing mounted-test diagnostic, run materializer and GlobalSearch focused tests, then exact full-tests. Do not add timeout or single-thread the test fixture.
- Evidence: the deterministic RED failed because source 10 entered `persist` while source 9 was active and therefore published `CanonicalSearchResult.Failure`. GREEN confines a `Mutex.withLock` to `persist`; the materializer RED then passed. The existing non-cancellable stale-generation test was corrected to release the older serialized transaction before expecting the replacement transaction, while retaining rejection of the stale completion and the new failure/retry flow. GlobalSearch production wiring 3/3 and SourceBrowse canonical wiring 3/3 passed; the mounted timeout diagnostic now prints every materializer terminal kind and failure cause. Root Spotless, diff check and Comet plan guard passed.
- [x] **Step 7: 独立批次与最终审查**
