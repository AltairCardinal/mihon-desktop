...
设置、外观、无障碍与许可施工已由 `completed-settings-child-plan` 完成。父 Task 6 的最终审计 child plan 也已完成，`active-child-plan` 已收口为 `none`；保留的 `original-roadmap` 与各 completed child plan 仅作为历史和证据入口，不再是活动施工权威。
...
- [x] Task 6：删除重复实现与最终审计

## 正文 checkbox 回填证据

| 父 Task | 已有执行权威与提交/验证证据 |
|---|---|
| Task 0 | 最终审计 child plan Tasks 1–5：64 项 manifest、fixed-main provenance、显式 closure RED→GREEN 与 protection aggregation；提交和测试数记录在各 Task execution evidence。 |
| Task 1A | 最终审计 Tasks 6、14、16C、17、18 与 product-parity closure child plan：共享状态/错误/偏好、DI、架构守卫及两端 consumer 均闭合。 |
| Task 1B | 最终审计 Tasks 6、10、16B、17、18 与 background-task child plan：网络错误、持久任务、通知/应用内反馈和恢复链均有 production fixture。 |
| Task 2A | 最终审计 Tasks 12、15、17、18：固定原版备份、Desktop 历史/canonical writer、恢复与自动备份 fixture 均通过。 |
| Task 2B | 最终审计 Tasks 10、14、17、18：下载、更新、历史和阅读进度共享/持久链及 Test Mode 场景均通过。 |
| Task 3A | 最终审计 Tasks 7、14、17、18 与 product-parity closure child plan：书库/详情入口、批处理、封面和统计 production wiring 均通过。 |
| Task 3B | 最终审计 Tasks 11、14、17、18：迁移编排、tracker 服务/凭据/同步与失败反馈均通过。 |
| Task 4A | `2026-07-15-mihon-reader-shared-core.md` 与最终审计 Tasks 8、9、17、18：固定原版 reader 语义、两端 consumer 及 Desktop 永久增强均通过。 |
| Task 4B | `2026-07-15-mihon-source-extension-shared-core.md` 与最终审计 Tasks 8、12、16A、17、18：源/扩展/挑战、真实 APK fixture、compat removal audit 均通过。 |
| Task 5A | `2026-07-21-mihon-desktop-platform-integration.md` 与 `2026-07-21-mihon-desktop-platform-verify.md`：系统集成、隐私和更新 adapter 已完成并复核。 |
| Task 5B | `2026-07-22-mihon-desktop-settings-accessibility.md` 与 `2026-07-23-mihon-desktop-settings-accessibility-verify.md`：设置、主题、无障碍与许可入口已完成并复核。 |
| Task 6 | `2026-07-23-mihon-desktop-final-parity-audit.md` 与最终验证报告：63 VERIFIED / 1 EXEMPT，Windows/macOS 同一 `19a55d7c` 版本均为 13/13、5/5、64/64、0 unmapped。 |

所有表内 child plan 均在正文记录各批提交 hash、focused/integration/full-suite 结果和独立审查结论；最终 runtime source commit 为 `19a55d7c27e854a9a5b8baa27871b6d8e1c3608c`。本文不复制第二份逐项 manifest。
...
- [x] **Step 1: 为错误映射、偏好默认值和旧 key 迁移写失败测试**
- [x] **Step 2: 运行 common 与 Desktop 精确测试，确认因类型/迁移缺失而 RED**
- [x] **Step 3: 从固定原版快照的调用链提取错误与任务状态语义，映射为 `AppError` 与 `TaskState`，禁止包含 Android 类型；不得把当前 `app/` 新增状态当成原版事实**
- [x] **Step 4: 将 `java.util.prefs` 封装成 PreferenceStore backend，保留旧 key 双读迁移**
- [x] **Step 5: 按 settings/reader/library/network/download/backup/extension 拆分 DI 注册函数**
- [x] **Step 6: 写 DI 全解析测试，覆盖每个 UI 中使用的依赖**
- [x] **Step 7: 逐 ScreenModel 切换共享状态，Composable 只 collect state/发送 intent**
- [x] **Step 8: 运行 `:domain:allTests`、`:core:common:allTests` 和 `:app-desktop:jvmTest`**
- [x] **Step 9: 删除被 PreferenceStore 覆盖的直接 `java.util.prefs` 业务访问**
- [x] **Step 10: 更新追踪项 3、4、7、12、93、95、96 的原版 provenance、消费证据和状态**
...
- [x] **Step 1: 写任务持久恢复、取消和通知映射失败测试**
- [x] **Step 2: 用临时目录模拟进程重启，确认当前 Desktop 内存 scheduler 无法恢复而 RED**
- [x] **Step 3: 从固定原版快照提取任务定义、约束和幂等规则到当前 shared core；当前 Android 构建版的 WorkManager 只作为消费端和平台 adapter 取证**
- [x] **Step 4: 实现 Desktop checkpoint store 与启动恢复，不复制 WorkManager API**
- [x] **Step 5: 实现系统通知 adapter；不可用时回退到应用内通知并记录原因**
- [x] **Step 6: 让 LibraryUpdateScheduler 消费共享任务，不再自有业务状态机**
- [x] **Step 7: 用 MockWebServer 验证离线、403、429、500、畸形响应的统一 AppError**
- [x] **Step 8: 运行网络、任务、更新和 Desktop wiring 测试，并用带原版 provenance 的行为向量验证 shared 默认值**
- [x] **Step 9: 删除重复重试/错误字符串解析和仅内存任务状态**
- [x] **Step 10: 更新追踪项 8、10、11、61 的原版 provenance、消费证据和状态**
...
- [x] **Step 1: 使用固定原版 Mihon 构建保存真实完整备份并记录 commit/版本/生成路径；另保存旧 Desktop 备份 fixture**
- [x] **Step 2: 写跨端读写失败测试：Desktop 读取原版备份；当前 Android 构建版和 shared codec 读取 Desktop 转换结果**
- [x] **Step 3: 运行 RED，确认固定原版 protobuf、历史 Desktop protobuf 与 canonical protobuf 的兼容边界**
- [x] **Step 4: 将固定原版 codec/validator 中的平台无关语义迁到 data common；不得从当前 Android writer 的新行为反推原版 schema**
- [x] **Step 5: 当前 Android 构建版 creator 与 Desktop creator 统一为只写 canonical protobuf**
- [x] **Step 6: 通过共享 codec 读取固定原版备份、具有明确版本 provenance 的其他原版历史备份与历史 Desktop protobuf，并映射到 canonical 共享模型**
- [x] **Step 7: 对漫画、章节、分类、历史、追踪、偏好、源、扩展仓库逐字段断言，期望值以原版 fixture 和显式 Desktop legacy 迁移规则为来源**
- [x] **Step 8: 为损坏、未知版本、部分恢复、取消和磁盘不足写集成测试**
- [x] **Step 9: UI 显示预览、进度、逐项结果和可恢复错误**
- [x] **Step 10: 运行当前 Android backup tests、data tests 和 Desktop backup tests；另验证原版 fixture provenance，不能把当前 Android 测试称为原版测试**
- [x] **Step 11: 删除旧 Desktop writer；通过共享 codec 保留固定原版、其他有 provenance 的原版历史备份与历史 Desktop protobuf 读取兼容**
- [x] **Step 12: 更新追踪项 71–74 的原版 schema/fixture、当前 Android consumer、Desktop consumer 和状态**
...
- [x] **Step 1: 从固定原版快照建立状态转换、公平调度、重试、自动下载和阅读进度 fixture，并为当前 Desktop 差异写失败测试**
- [x] **Step 2: 为进度/history/已读/tracker event 的单事务语义写失败测试；期望结果必须追溯到原版行为或单独批准的 bugfix**
- [x] **Step 3: 运行 RED，确认 Desktop 内存队列和独立 progress tracker 暴露差异**
- [x] **Step 4: 从固定原版快照提取队列状态机、自动下载规则和 retry/backoff；先比较当前 `app/` 与原版，禁止从当前 Android 构建版新增逻辑反推原版**
- [x] **Step 5: 实现 SQLDelight/持久 store，并迁移当前 Desktop queue snapshot**
- [x] **Step 6: Desktop manager 收敛为文件下载 adapter，不再决定业务状态转换**
- [x] **Step 7: Updates/History ScreenModel 直接消费共享 use case 和下载状态**
- [x] **Step 8: ReaderProgressTracker 改为提交共享阅读事件，事务内联动 tracker**
- [x] **Step 9: 保留 CBZ、目录选择、Upcoming 与 Test Mode 回归**
- [x] **Step 10: 运行 shared、当前 Android 下载/历史测试、Desktop 集成测试和 E2E；当前 Android 测试只证明当前消费者 wiring**
- [x] **Step 11: 删除旧内存队列/重复自动下载和进度规则**
- [x] **Step 12: 更新追踪项 53、56、57、59、62、64 的四方证据和状态**
...
- [x] **Step 1: 从固定原版快照提取分类排序、筛选组合、范围选择和部分失败 fixture，写共享契约测试**
- [x] **Step 2: 写 Desktop 鼠标/Shift 多选、宽屏布局、作者入口保护测试**
- [x] **Step 3: 运行 RED，记录 Desktop 重写与固定原版结果差异；当前 Android 构建版只作为额外差分对象**
- [x] **Step 4: 将固定原版中缺失于 shared 的规则迁入共享 use case；当前 shared 中已与原版一致的 use case 直接接线**
- [x] **Step 5: ScreenModel 只组合共享 state，UI 只发送 intent**
- [x] **Step 6: 封面文件选择通过 adapter，缓存失效/错误使用共享模型**
- [x] **Step 7: StatsScreen 消费共享统计聚合，不在 Composable 查询/计算**
- [x] **Step 8: 补 Screen 实例化、导航和 DI wiring 测试**
- [x] **Step 9: 运行 library/detail/stats 与 Desktop 产品保护测试**
- [x] **Step 10: 删除 DesktopCategoryManager 等已被共享实现完全覆盖的业务类**
- [x] **Step 11: 更新追踪项 16、17、19、22、24、26、66 的原版 provenance、shared 和两端消费证据**
...
- [x] **Step 1: 从固定原版快照为迁移选项、章节匹配、分类/阅读状态复制写契约测试**
- [x] **Step 2: 为批量取消、恢复点和逐项失败写 RED 测试**
- [x] **Step 3: 从固定原版快照抽取迁移编排语义，Desktop use case 改为薄调用器；先审计当前 Android 构建版是否已偏离原版**
- [x] **Step 4: 将固定原版 tracker API/domain 中平台无关逻辑迁到当前 shared source set；当前 `app/.../track` 改为 Android adapter/consumer**
- [x] **Step 5: 实现 loopback OAuth callback 和 OS credential store adapter**
- [x] **Step 6: 新增 Tracker 设置、登录、搜索绑定、状态/分数/章节编辑 UI**
- [x] **Step 7: ReaderProgress 事件接入从固定原版提取的自动同步策略和重试任务**
- [x] **Step 8: MockWebServer 覆盖每类认证、刷新 token、429、500 和畸形响应**
- [x] **Step 9: 保护作品比较、宽屏迁移队列和 Test Mode**
- [x] **Step 10: 运行 migration/tracker/reader shared、当前 Android consumer 和 Desktop integration tests**
- [x] **Step 11: 删除 Desktop 重复迁移业务规则**
- [x] **Step 12: 更新追踪项 67–70 的原版 tracker provenance、shared 和两端消费证据**
...
- [x] **Step 1: 只从固定原版快照的页面模型、viewer/loader 和测试建立真实测试向量；运行 `git diff <original-ref>..<task-base>` 排除当前分支新增的 Android 双页/配对行为**
- [x] **Step 2: 写宽页反转/旋转、缺章、重试、预加载取消、导航反转、灰度/反色 RED 测试；每项期望标明原版符号或另行批准的 bugfix**
- [x] **Step 3: 写 Desktop 双页、edge matching、自动滚动、键鼠、右键保存保护测试；仅存在于当前 Android 构建版而不在原版中的行为也先归为 fork 增强，不进入 shared 默认**
- [x] **Step 4: 将固定原版中的纯页面模型/算法语义迁到 domain common，不移动 Android View；原版没有的增强通过显式 options 或独立 bugfix 表达**
- [x] **Step 5: 定义 `PageDecoder`/`RegionDecoder`/`PageCache` 平台接口**
- [x] **Step 6: Desktop 使用 Skia codec/tiles 实现区域解码和内存预算**
- [x] **Step 7: 当前 Android 构建版 viewer 与 Desktop JVM viewer 同时消费共享状态/命令；当前 Android wiring 不得被称为原版实现**
- [x] **Step 8: 用同一原版章节 fixture 比较原版期望、shared 结果、当前 Android consumer 与 Desktop consumer 的页序、过渡、跳过和错误结果**
- [x] **Step 9: 运行内存/大图测试，验证不会全尺寸长期驻留**
- [x] **Step 10: 运行 reader common、当前 Android reader、Desktop reader 和 Test Mode 测试；当前 Android 测试只作为消费者证据**
- [x] **Step 11: 删除 Desktop 重复拆页、导航和跳过规则；保留渲染/输入增强及显式分类的 fork 增强**
- [x] **Step 12: 更新追踪项 9、43、44、45、47、49、51、54 的原版向量、shared 默认、当前 Android/Desktop 消费及有意偏差**
...
**Completion status:** 父 Task 5B 与子计划 Tasks 1–20/20A–20F 已完成。whole-change 唯一修复复审 APPROVED `0/0/0`；Desktop full `2102/2102`，Windows 与 macOS 阶段版本 `0.11.14.45.5a02aed` 已构建并运行。Android API36 首轮运行暴露由两个跨模块同名 `ThemeModeKt` facade 引起的 `NoSuchMethodError`；Task 20F 以独立 Android facade 修复，复审 `0/0/0`，修复后 production 冷启动、instrumentation `5/5`、focused `8/8` 及设置/主题/语言/许可运行态全部通过。该阶段结束时 ID88 仍按真实证据保持 CHARACTERIZED；后续最终审计补齐 production accessibility consumer/protection 后，ID88 与 90/91/94 均已为 VERIFIED。macOS VoiceOver/AX/真实键盘与 Android TalkBack 仍是人工辅助验收边界，不影响已执行的 production wiring 契约。

**Verification report:** `docs/superpowers/reports/2026-07-23-mihon-desktop-settings-accessibility-verify.md`

### Task 6：删除重复实现与最终审计

**Completion evidence:** 本 Task 已由
`docs/superpowers/plans/2026-07-23-mihon-desktop-final-parity-audit.md` 完成。64 项终态 gate、唯一
EXEMPT 用户批准链、compat/重复规则/架构/Test Mode 审计、Windows/macOS 同版本最终构建与运行
验收均通过；父 Task 0–5B 正文 checkbox 已按上表证据回填。最终报告为
`docs/superpowers/reports/2026-07-23-mihon-desktop-final-parity-verify.md`，根 coverage report 为
...
- [x] **Step 1: 扫描 64 项追踪矩阵，确认每项为 VERIFIED 或 EXEMPT，且 `originalRef` 与本计划一致**
- [x] **Step 2: 对每个 EXEMPT 检查 OS 能力证据、UI 边界说明和用户批准记录**
- [x] **Step 3: 建立重复实现审计表，逐项对照固定原版 ref/符号、迁移后 shared 实现、当前 Android consumer、Desktop consumer 与平台 adapter 职责**
- [x] **Step 4: 为不允许的 UI→data/network/manager 依赖增加架构测试**
- [x] **Step 5: 删除已无调用的临时 adapter、旧 writer、重复状态机和无证据 Desktop Android API shim**
- [x] **Step 6: 运行 `./gradlew spotlessCheck`**
- [x] **Step 7: 运行相关 shared 与当前 Android 单元/集成测试；测试输入必须有原版 provenance，当前 Android 测试不得单独充当原版一致性证明**
- [x] **Step 8: 运行 `./gradlew :app-desktop:jvmTest` 和 `./gradlew :test-desktop:test`**
- [x] **Step 9: 运行 Desktop smoke test/Test Mode 全场景，核验保护清单零回退**
- [x] **Step 10: 仅由 child Task 19 使用 `scripts/build-desktop.sh` 生成最终待验收 BUILD**
- [x] **Step 11: 仅在 Task 19 启动固定未打包 Windows EXE，并对同一提交的 macOS app bundle 完成最终一次运行验收**
- [x] **Step 12: 更新比较报告：重新以固定原版快照比较，64 项改为已对齐、有意 cross-platform bugfix 或有证据的平台豁免**
- [x] **Step 13: 在完成报告中列出原版 ref、完整 Desktop 版本、EXE 路径、测试命令、失败数、剩余有意偏差和豁免**
