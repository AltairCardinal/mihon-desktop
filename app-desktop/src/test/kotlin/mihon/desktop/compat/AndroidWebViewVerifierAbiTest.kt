...
class AndroidWebViewVerifierAbiTest {
    ...
    fun `WebView verifier tokens keep exact shapes while engine methods fail fast`() {
        ...
        assertEquals(2, uri.declaredMethods.size)
        assertEquals(setOf("toString", "encode"), uri.declaredMethods.map { it.name }.toSet())
        val uriEncode = uri.getMethod("encode", String::class.java)
        assertTrue(Modifier.isPublic(uriEncode.modifiers))
        assertTrue(Modifier.isStatic(uriEncode.modifiers))
        assertEquals(listOf(String::class.java), uriEncode.parameterTypes.toList())
        assertEquals(String::class.java, uriEncode.returnType)
        assertEquals("AZaz09_-!.~'()*", uriEncode.invoke(null, "AZaz09_-!.~'()*"))
        assertEquals("a%20b", uriEncode.invoke(null, "a b"))
        assertEquals("%F0%9F%98%80", uriEncode.invoke(null, "😀"))
